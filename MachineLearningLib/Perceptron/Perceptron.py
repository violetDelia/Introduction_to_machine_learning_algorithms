...
from ModelEvaluation.ModelEvaluation import EvaluationModel
from DataVisualization.DateVisualization import DataVisualization


class Perceptron(PerceptronNonexact, EvaluationModel, DataVisualization):
    ''' 
    感知机:
    ...
    def __init__(self, learning_rate=1.0, w_start=None, b_start=None, max_steps=3000,
    ...
    def train(self, X_train, y_train, type=SoulutionType.normal):
        ...
        if type == self.SoulutionType.normal_nonexact:
            self._fit_nonexact(X_train, y_train)
    ...
    def predict(self, X_test):
        ...
        if self.w is None or self.b is None:
            return None
        y_predict = self._predict(X_test, self.w, self.b)
        return y_predict
